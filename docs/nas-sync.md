...
# Music
~/.local/bin/sync/nas/sync-music

# Photos
~/.local/bin/sync/nas/sync-photos

# Audiobooks
~/.local/bin/sync/nas/sync-audiobooks
```
...
```bash
~/.local/bin/sync/nas/check-nas-connection
```
...
```bash
cat > ~/.local/bin/sync/nas/sync-newdir << 'EOF'
#!/usr/bin/env bash
# Sync NewDir with NAS
exec "$HOME/.local/bin/sync/nas/sync-to-nas" "$HOME/NewDir" "newdir" "nate"
EOF
chmod +x ~/.local/bin/sync/nas/sync-newdir
```
...
[Service]
Type=oneshot
ExecStart=%h/.local/bin/sync/nas/sync-newdir
...
**Scripts:**
- `~/.local/bin/sync/nas/sync-to-nas` - Main sync script
- `~/.local/bin/sync/nas/sync-documents` - Documents wrapper
- `~/.local/bin/sync/nas/sync-music` - Music wrapper
- `~/.local/bin/sync/nas/sync-photos` - Photos wrapper
- `~/.local/bin/sync/nas/sync-audiobooks` - Audiobooks wrapper
- `~/.local/bin/sync/nas/check-nas-connection` - Connectivity check
...
**Dotfiles:**
- Source: `~/dotfiles/home/.local/bin/sync/nas/`
